$(function() {
	...
	// セレクトボックス要素
	var option = {
		count_10: '<option value="10">10</option>',
		count_20: '<option value="20">20</option>',
		count_81: '<option value="81">81</option>',
	}
	...
	// レベル選択時等に回数のプルダウン初期化する
	var updateSelCount = function(selVal, levelVal, countVal) {
		$('#sel_count').empty();
		if (selVal == 3 && levelVal == 1) {
			$('#sel_count').append(option.count_10);
			$('#sel_count').append(option.count_20);
			$('#sel_count').append(option.count_81);
			$('#sel_count').val(countVal);
		} else {
			$('#sel_count').append(option.count_10);
			$('#sel_count').append(option.count_20);
			if (countVal > 20) {
	...
	$('#sel_div').change(function() {
		updateSelCount(this.value, $('#sel_level').val(), $('#sel_count').val());
		showHiscoreGrid();
	});
	$('#sel_level').change(function() {
		updateSelCount($('#sel_div').val(), this.value, $('#sel_count').val());
		showHiscoreGrid();
	});
	...
	var showHiscoreGrid = function() {
		$('#lbl_hiscore').text('ハイスコア(' + $('#sel_div option:selected').text() + ':レベル' + $('#sel_level option:selected').text() + ':問題数' + $('#sel_count').val() + ')');
		$('#grid_hiscore').jsGrid({
			width: '100%',
	...
	$('#btn_start').on('click', function(event) {
		...
		if (count == 81) {
			// 九九の場合は先に問題抽選を済ませる
			kukuQuestList = [];
			for (var numA = 1; numA <= 9; numA++) {
				for (var numB = 1; numB <= 9; numB++) {
					kukuQuestList.push({ questTxt: numA + ' × ' + numB + ' = ', answer: numA * numB  });
				}
			}
			shuffle(kukuQuestList);
		}
		...
		while (true) {
			if (level == 1) {
				if (div == 1) {
					...
					if (count == 81) {
						// 九九の場合
						questTxt = kukuQuestList[questCnt - 1].questTxt;
						answer = kukuQuestList[questCnt - 1].answer;
					} else {
						var numA = Math.floor(Math.random() * 9) + 1;
						var numB = Math.floor(Math.random() * 9) + 1;
						questTxt = numA + ' × ' + numB + ' = ';
	...
	var pressAnswer = function() {
		if (isActive) {
			...
			if (answer == $('#lbl_answer').text()) {
				...
				$('#lbl_judge').text('×');
				if ((count == 81 && point > 0) || point > 1) {
					// 間違えるごとに減点。最低1点はもらえるが、99の場合は0点まで下がる
					point--;
				}
				setTimeout(function() {
					$('#lbl_answer').text('');
	...
	updateSelCount($('#sel_div').val(), $('#sel_level').val(), $('#sel_count').val());
	showHiscoreGrid();
	initArray();
});
