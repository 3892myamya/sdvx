package myamya.other.solver.maxi;
...
public class MaxiSolver implements Solver {
	...
	public static class Field {
		...
		public boolean roomSolve() {
			for (Room room : rooms) {
				if (room.getWhiteCnt() != -1) {
					// 部屋に対する調査
					Set<Set<Position>> overPosSetSet = new HashSet<>();
					...
					for (Position pos : room.getMember()) {
						if (!alreadyPosSet.contains(pos)) {
							...
							posSet.add(pos);
							setContinueRoomPosSet(pos, posSet, null);
							if (posSet.size() >= room.getWhiteCnt()) {
								sizeExists = true;
								if (posSet.size() > room.getWhiteCnt()) {
									overPosSetSet.add(posSet);
								}
							}
							alreadyPosSet.addAll(posSet);
						}
					}
					// サイズ不足
					if (!sizeExists) {
					...
					for (Set<Position> overPosSet:overPosSetSet) {
						Set<Position> alreadyOverPosSet = new HashSet<>();
						for (Position pos : overPosSet) {
							if (!alreadyOverPosSet.contains(pos)) {
								Set<Position> posSet = new HashSet<>();
								posSet.add(pos);
								setContinueRoomWhitePosSet(pos, posSet, null);
								// サイズ超過
								if (posSet.size() > room.getWhiteCnt()) {
									return false;
								}
								alreadyOverPosSet.addAll(posSet);
							}
						}
					}
				}
		...
		/**
		 * posを起点に上下左右に壁なし確定マスをつなげていく。
		 * 国境をまたぐこともできないものとする。
		 */
		private void setContinueRoomWhitePosSet(Position pos, Set<Position> continuePosSet, Direction from) {
			if (pos.getyIndex() != 0 && from != Direction.UP) {
				Position nextPos = new Position(pos.getyIndex() - 1, pos.getxIndex());
				if (tateWall[pos.getyIndex() - 1][pos.getxIndex()] == Wall.NOT_EXISTS
						...
						&& !continuePosSet.contains(nextPos)) {
					continuePosSet.add(nextPos);
					setContinueRoomWhitePosSet(nextPos, continuePosSet, Direction.DOWN);
				}
			}
			if (pos.getxIndex() != getXLength() - 1 && from != Direction.RIGHT) {
				Position nextPos = new Position(pos.getyIndex(), pos.getxIndex() + 1);
				if (yokoWall[pos.getyIndex()][pos.getxIndex()] == Wall.NOT_EXISTS
						...
						&& !continuePosSet.contains(nextPos)) {
					continuePosSet.add(nextPos);
					setContinueRoomWhitePosSet(nextPos, continuePosSet, Direction.LEFT);
				}
			}
			if (pos.getyIndex() != getYLength() - 1 && from != Direction.DOWN) {
				Position nextPos = new Position(pos.getyIndex() + 1, pos.getxIndex());
				if (tateWall[pos.getyIndex()][pos.getxIndex()] == Wall.NOT_EXISTS
						...
						&& !continuePosSet.contains(nextPos)) {
					continuePosSet.add(nextPos);
					setContinueRoomWhitePosSet(nextPos, continuePosSet, Direction.UP);
				}
			}
			if (pos.getxIndex() != 0 && from != Direction.LEFT) {
				Position nextPos = new Position(pos.getyIndex(), pos.getxIndex() - 1);
				if (yokoWall[pos.getyIndex()][pos.getxIndex() - 1] == Wall.NOT_EXISTS
						...
						&& !continuePosSet.contains(nextPos)) {
					continuePosSet.add(nextPos);
					setContinueRoomWhitePosSet(nextPos, continuePosSet, Direction.RIGHT);
				}
			}
		}
	...
	public MaxiSolver(int height, int width, String param) {
	...
	public static void main(String[] args) {
		...
		System.out.println(new MaxiSolver(height, width, param).solve());
	}
